...
export default function LegalPage({ type, setView }: LegalPageProps) {
  ...
  return <article className="mx-auto max-w-3xl px-6 py-14 text-slate-700">
    ...
    {privacy ? <div className="space-y-6 leading-relaxed">
      ...
      <section><h2 className="text-xl font-bold text-brand-blue-dark">No vendemos tus datos</h2><p>No vendemos, alquilamos ni usamos información personal para publicidad dirigida. Solo la compartimos con proveedores técnicos indispensables —infraestructura, base de datos y correo— bajo la finalidad de operar el servicio, o cuando exista una obligación legal válida.</p></section>
      <section><h2 className="text-xl font-bold text-brand-blue-dark">Proveedores y conservación</h2><p>El servicio utiliza infraestructura administrada por el responsable, PostgreSQL y el proveedor de correo configurado. Conservamos la cuenta mientras esté activa y los registros técnicos durante el plazo necesario para seguridad, auditoría y obligaciones aplicables. Al eliminar una cuenta, sus datos desaparecen de la base activa; las copias de seguridad cifradas expiran conforme al calendario de retención y solo se restauran para recuperación ante incidentes.</p></section>
      <section><h2 className="text-xl font-bold text-brand-blue-dark">Tus derechos y controles</h2><p>Desde Seguridad de la cuenta puedes descargar una copia de tus datos o eliminar permanentemente la cuenta. También puedes solicitar acceso o corrección contactando al correo de privacidad publicado para el servicio.</p></section>
